...
> **Correction du 09-08-2026 :** le corpus courant contient **35 identifiants normatifs uniques**.
> La matrice a 30 lignes brutes, car sa ligne agrégée `US-G1-SSH-001..005,008` représente six
> headings SSH et non une seule story. Après dépliage de cette ligne, les 35 IDs de la matrice et les
> 35 headings normatifs concordent ; le compteur antérieur de 31 était erroné.
...
| US-G0-UX-004 | §5.6 — capture Web `/tasks/new` vers brouillon attribué, sans lancement | US-G0-UX-001, identité Console, `guided.task.create` | — | `PROPOSÉE` — optionnelle, non bloquante pour le GO Gate 0 ; aucune dépendance de Gate ni ajout aux dépendances de `US-G0-001..003` ; capture rapide crée ou enrichit seulement un brouillon |
...
| US-G0-INBOX-001 | §5.6, §6.2 — file partagée par rôle/capacité, sans tracker ni autorité de décision | US-G0-TASK-001, US-G0-APPROVAL-001 | [annexe Inbox paginée 09-08-2026](evidence/2026-08-09-inbox-summary-and-guided-create-migration.md), `bun test apps/web/src/lib/inbox.test.ts apps/web/src/loaders.test.ts apps/web/src/components/shell/nav-config.test.ts apps/web/src/screens/inbox.test.tsx` ; `bun run --filter web typecheck` ; navigateur réel `127.0.0.1:1420/inbox` (preuve historique, header partagé, section brouillons, nom projet humain, nav de création) ; P-E2E multi-persona/source dégradée et revue absents | `VÉRIFIÉE LOCALEMENT, NON ACCEPTÉE` — brouillon séparé, contexte projet humain porté par le résumé serveur, wording partagé, gate `guided.task.create`, personas et deux sources dégradées couverts localement. La preuve navigateur historique ne couvre pas les personas négatifs ni les pannes de source ; revue d’acceptation requise, sans requalifier `UX-001`, `TASK-001` ni `DELIVERY-001` |
| US-G0-INBOX-002 | §5.6, §6.2 — affectation personnelle par résumé serveur, sans assignee picker | US-G0-INBOX-001 ; US-G0-UX-004 pour les brouillons issus de capture rapide | — | `PROPOSÉE` — optionnelle, non bloquante pour le GO Gate 0 ; aucune dépendance de Gate ni ajout aux dépendances de `US-G0-001..003` ; attribution serveur et P-E2E affecté/non affecté requis |
