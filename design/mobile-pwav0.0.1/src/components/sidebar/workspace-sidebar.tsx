...
/** Lab module: design surfaces, kept out of the product vocabulary. */
...
const MODULES = [
  ...
  { id: "channels", label: "Salons", icon: RiHashtag, home: "/channels/general" },
  { id: "lab", label: "Lab", icon: RiLayout4Line, home: "/layouts" },
] as const;
...
function moduleForPath(pathname: string): ModuleId {
  ...
  if (pathname.startsWith("/layouts")) return "lab";
  return "console";
}
  ...
  return (
    ...
    >
      <form
        ...
        onSubmit={submit}
        noValidate
        data-pwa-dirty={name ? "true" : undefined}
      >
  ...
  if (collapsed)
    return (
      <button
        ...
        className="inline-flex size-11 self-center items-center justify-center rounded-[10px] bg-transparent text-[var(--muted-foreground)] transition-colors hover:bg-[var(--surface-hover)] hover:text-[var(--foreground)] lg:size-10"
      >
        {dark ? (
...
export function WorkspaceSidebar({
  ...
  const activeLabel =
    MODULES.find((module) => module.id === activeModule)?.label ?? "Console";
  ...
  const navRow = ({ label, to, icon: Icon }: NavEntry) => (
    <NavLink
      ...
      className={({ isActive }) =>
        `flex h-8 w-full items-center gap-2 overflow-hidden rounded-[8px] px-2 transition-colors ${isActive ? "bg-[image:var(--gradient-primary)] text-[var(--accent-contrast)] shadow-[var(--shadow-nav-selected)]" : "text-[var(--muted-foreground)] hover:bg-[var(--surface-hover)] hover:text-[var(--foreground)]"}`
      }
    >
      <Icon className="size-4 shrink-0" />
      <span className="truncate text-body-2-medium">{label}</span>
    </NavLink>
  );
  return (
    <>
      ...
      <div
        aria-hidden
        className={`hidden shrink-0 transition-[width] duration-200 ease-linear lg:block ${collapsed ? "lg:w-16" : "lg:w-[304px]"}`}
      />
      <aside
      ...
      >
        <div className="workspace-sidebar-panel flex h-full bg-[var(--sidebar)]">
          ...
          >
            <div className="flex h-full w-60 flex-col justify-between pl-2">
              <div className="flex min-h-0 flex-1 flex-col">
                <div className="flex h-10 shrink-0 items-center px-2">
                  <span className="text-caption-1 font-semibold tracking-[0.04em] text-[var(--muted-foreground)] uppercase">
                    {activeLabel}
                  </span>
                </div>
                <button
                  ...
                  className="flex h-8 w-full items-center gap-2 rounded-full bg-[var(--control)] px-2 text-[var(--muted-foreground)] transition-colors hover:bg-[var(--surface-hover)] hover:text-[var(--foreground)]"
                >
                  <span className="flex min-w-0 flex-1 items-center gap-2">
                    <RiSearchLine className="size-4 shrink-0" />
                    <span className="truncate text-body-2-medium">Rechercher</span>
                  </span>
                  <kbd className="rounded-[4px] bg-[var(--surface-raised-hover)] px-1 text-caption-1 font-medium text-[var(--text-tertiary)]">
                    ⌘L
                  </kbd>
                </button>
                <div className="flex min-h-0 flex-1 flex-col overflow-y-auto pt-3 overscroll-contain">
