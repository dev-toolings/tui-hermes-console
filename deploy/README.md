...
containing the SSH configuration and identity required by the `hermes-console`
service account. Apply the opt-in overlay:
...
The overlay mounts that directory read-only at `/home/bun/.ssh`. Host keys
verified from the Runtime Hermes screen are stored separately in the persistent
`ssh-data` volume at `/data/ssh/known_hosts`, writable only by the `bun` service
user. No private key belongs in Git. The SSH config must use a deterministic
`IdentityFile` readable by UID `bun` because the runtime enforces `BatchMode=yes`,
