  ...
  return (
    <form
      onSubmit={async (event) => {
        ...
        if (workspaceBlocked) {
          setError("Configurez le dossier de travail distant avant de lancer une mission.");
          return;
        }
        setSubmitting(true);
        setError(null);
    ...
    >
      {workspaceBlocked ? (
        <div className="flex flex-wrap items-center gap-3 rounded-xl bg-warn-soft p-3 text-[0.75rem] text-warn-700">
          ...
          <p className="min-w-0 flex-1">
            Connexion SSH valide, mais le dossier de travail Hermes reste à configurer.
          </p>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-t border-seam pt-4">
        ...
        <Button type="submit" variant="primary" disabled={workspaceBlocked || submitting || agents.length === 0}>
          {submitting ? (
