...
export const XuluxComposer: FC<{ modelLabel?: string }> = ({ modelLabel }) => (
    ...
    <ComposerPrimitive.AttachmentDropzone asChild>
      <div data-slot="aui_composer-shell" className={composerShellClass}>
        <ComposerPrimitive.Input
          ...
          aria-label="Message"
        />
        <XuluxComposerAction modelLabel={modelLabel} />
      </div>
    </ComposerPrimitive.AttachmentDropzone>
  </ComposerPrimitive.Root>
);

const XuluxComposerAction: FC<{ modelLabel?: string }> = ({ modelLabel }) => {
  const shortModel = formatXuluxModelLabel(modelLabel);
  ...
  return (
    <div className="aui-composer-action-wrapper relative flex items-center justify-between">
      <div className="flex items-center gap-1">
        ...
        {shortModel ? (
          <span className="text-muted-foreground inline-flex h-7 max-w-[10rem] items-center truncate rounded-full bg-muted/80 px-2.5 text-xs font-medium">
            {shortModel}
          </span>
        ) : null}
      </div>
